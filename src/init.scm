...
(define comparator-exit-iternal (lambda ()
           (error 'generic-comparator
             "all the arguments must be numbers")))

(define make-bin-comparator-iternal (lambda (comparator-qq comparator-rr)
           (lambda (a b)
             (cond ((rational? a)
...
(define make-run-comparator-iternal (lambda (a s)
                         (or (null? s)
                             (and (bin-ordering a (car s))
                                  (run (car s) (cdr s))))))

(let ((exit comparator-exit-iternal)
       (bin<? (make-bin-comparator-iternal __bin-less-than-qq __bin-less-than-rr))
       (bin=? (make-bin-comparator-iternal __bin-equal-qq __bin-equal-rr))
       (bin>=? (lambda (a b) (not (bin<? a b))))
       (bin>? (lambda (a b) (bin<? b a)))
       (bin<=? (lambda (a b) (not (bin>? a b)))))
  (let ((make-run
          (lambda (bin-ordering)
            (let ((run make-run-comparator-iternal))
              (lambda (a . s) (run a s))))))
    (set! < (make-run bin<?))
    (set! <= (make-run bin<=?))
    (set! > (make-run bin>?))
