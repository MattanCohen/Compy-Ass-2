...
let rec gcd a b =
 ...
 module Reader : READER = struct
   ...
   and nt_paired_comment str =
     ...
     let nt2 = disj nt_paired_comment nt2 in 
     let nt3 = diff nt_any (disj (char '{') (char '}')) in
   ...
   and nt_sexpr_comment str =
     let nt1 = word "#;" in
     let nt2 = maybe nt_sexpr_comment in
     let nt2 = caten nt1 nt2 in
     let nt3 = caten nt2 nt_sexpr in
     let nt3 = unitify nt3 in
     nt3 str
   and nt_comment str =
     disj_list
   ...
   and nt_optional_sign str = 
     ...
     let nt2 = make_maybe nt1 '+' in
     let nt3 = pack nt2 (fun (s) ->
                         (Char.equal '+' s)) in
     nt3 str
   ...
   and nt_floatA str = 
     ...
     let maybe_expo = (make_maybe nt_exponent 1.0) in
     let nt1 = caten nt_integer_part (caten (char '.') (caten maybe_mantissa maybe_expo)) in
     let nt1 = pack nt1 
               (fun (integer_part, (dot, (mant, expo))) -> 
                 (integer_part +. mant) *. expo) in
     nt1 str
   
   and nt_floatB str = 
     let maybe_expo = (make_maybe nt_exponent 1.) in
     let nt1 = caten (char '.') (caten nt_mantissa maybe_expo) in
     let nt1 = pack nt1 (fun (dot, (mant, expo)) -> mant *. expo) in
     nt1 str
   ...
   and nt_float str = 
     let float_types = disj_list [nt_floatA; nt_floatB; nt_floatC] in
     let nt1 = caten nt_optional_sign float_types in
     let nt1 = pack nt1
                 (fun (is_positive, f) ->
                   if is_positive then ScmReal(f) else ScmReal(-.f)) in 
     nt1 str
   ...
   and nt_char_named str =
     let nt1 = disj_list [pack (word_ci "null") (fun _ -> '\000');
   ...
   and nt_symbol str =
     ...
     let nt1 = pack nt1 (fun chars ->
                          ScmSymbol (string_of_list chars)) in
     nt1 str
   and nt_string_part_simple str =
   (*any char thats not meta*)
     let nt1 =
   ...
   and nt_string_part_dynamic str = 
       ...
       let nt1 = pack nt1 (fun (_, (sexpr, _)) -> 
         Dynamic(ScmPair(ScmSymbol("format"), ScmPair(ScmString("~a"), ScmPair(sexpr, ScmNil))))) in 
       nt1 str
   ...
   and nt_vector str = 
     ...
     let nt_rb = caten nt_skip_star (char ')') in
     let nt3 = pack (caten nt1 (caten nt2 nt_rb)) (fun (_, (sexprs, (_,_))) -> ScmVector(sexprs)) in
     make_skipped_star nt3 str
 
 
   and nt_list str = 
     let nt_lb = char '(' in
     let nt_rb = char ')' in
     let nt2 = pack (caten (nt_skip_star) nt_rb) (fun x -> ScmNil) in
     let nt_sexp_plus = (plus nt_sexpr) in
     let nt4 = pack nt_rb (fun x -> ScmNil) in
     let nt5 = pack (caten (char '.') (caten nt_sexpr nt_rb)) 
                     (fun (_, (sexpr, _)) -> sexpr) in
     let nt_list_end = disj nt4 nt5 in
     let nt3 = pack (caten nt_sexp_plus nt_list_end) 
                     (fun (sexprs, sexpr) -> 
                       List.fold_right (fun car cdr -> ScmPair(car, cdr)) sexprs sexpr) in
     let nt2 = disj nt2 nt3 in
     let nt1 = pack (caten nt_lb nt2) (fun (_, sexpr) -> sexpr) in
...
module Tag_Parser : TAG_PARSER = struct
  ...
  let rec macro_expand_qq = function
   ...
   (*by def, slides 3/47*)
  let rec macro_expand_and_clauses sexpr = function
    | [] -> sexpr 
    | sexpr' :: sexprs -> 
      ScmPair(
         ScmSymbol "if",
         ScmPair(
            sexpr,
            ScmPair(
               macro_expand_and_clauses sexpr' sexprs,
               ScmPair(ScmBoolean(false), ScmNil))));;

  let rec macro_expand_cond_ribs = function
    | ScmNil -> raise (X_syntax "invalid syntax in cond")
  ...
  let rec macro_expand_letrec vars body =
    let whatever = ScmPair (ScmSymbol "quote",
      ScmPair (ScmSymbol "whatever", ScmNil)) in
    let rec arrange_letrec = function
    | [] -> (ScmNil, body)
    | ScmPair(id, sexpr)::rest ->
      let (r_init, r_set) = arrange_letrec rest in
      (ScmPair(ScmPair(id, ScmPair(whatever, ScmNil)), r_init),
       ScmPair(ScmPair(ScmSymbol("set!"),
                       ScmPair(id, sexpr)), r_set))
    | _ -> raise (X_syntax "malformed let expr") in
    let inits, sets = arrange_letrec vars in
    ScmPair (ScmSymbol "let", ScmPair (inits, sets))
  ...
  let rec tag_parse sexpr =
    ...
    | ScmPair (ScmSymbol "lambda", ScmPair (params, exprs)) ->
       ...
       (match (scheme_list_to_ocaml params) with
        | params, ScmNil ->
           ScmLambda(unsymbolify_vars params, Simple, expr)
    ...
    | ScmPair (ScmSymbol "let", ScmPair (vars, body)) ->
      ...
      | vars, ScmNil ->
          tag_parse (macro_expand_let vars body)
      | _ -> raise (X_syntax "invalid parameter list") )
    | ScmPair (ScmSymbol "let*", ScmPair (ScmNil, exprs)) ->
          tag_parse (macro_expand_let [] exprs)
    | ScmPair (ScmSymbol "let*",
               ScmPair
                 (ScmPair
                    (ScmPair (var, ScmPair (value, ScmNil)), ScmNil),
                  exprs)) -> tag_parse (macro_expand_let [ScmPair (var, ScmPair (value, ScmNil))] exprs)
    ...
    | ScmPair (ScmSymbol "let*",
          ...
          tag_parse (macro_expand_let [ScmPair (var, ScmPair (arg, ScmNil))] rest)
                        (* 
                          ...
                          ->
                            ...
                            (ScmPair (var,
                                          ScmPair (arg,
                                                   ScmNil))
                            letStar: ScmPair (ScmSymbol "let*", ScmPair(ribs, exprs)
                          macro_let    -rib1-    -tag_parse letStar-      
                            (let       (x 2)     (let*   (y 3)   x))
    ...
    | ScmPair (ScmSymbol "let*", ScmPair (ScmNil, exprs)) ->  macro_expand_let ScmNil exprs
                            ...
                            --------------macro_expand_let-----------
                              ---new rib---     ------------newExprs------------
                                                  -letStar-   -ribs.next-     -exprs-
                               (let (x 2)          (let*       (y 3)           x))
                            ->
                                        -parse newExprs-
                              (let (x 2) (let (y 3) x))

                              ((lambda (x) ((lambda (y) x) 3)) 2)
...
module Semantic_Analysis : SEMANTIC_ANALYSIS = struct
  ...
  let annotate_lexical_address =
    let rec run expr params env =
      ...
      | ScmVarSet(Var v, expr) ->
          let v_addr = tag_lexical_address_for_var v params env in
          ScmVarSet' (v_addr, (run expr params env))
      (* this code does not [yet?] support nested define-expressions *)
      | ScmVarDef(Var v, expr) ->
          ScmVarDef' ((Var' (v, Free)), (run expr params env))
      | ScmLambda (params', Simple, expr) ->
        let extenv = List.append [params] env in
         ScmLambda' (params', Simple, run expr params' extenv)
      | ScmLambda (params', Opt opt, expr) ->
         let params'' = List.append params' [opt] in
         ScmLambda' (params', Opt opt, run expr params'' (List.append [params] env))
      | ScmApplic (proc, args) ->
  ...
  let should_box_var name expr params =
    let r, w = find_reads_and_writes name expr params in
    let cross = cross_product r w in
    let same_occ pair = match pair with
    | ((Var' (_, Param _), _), (Var' (_, Param _), _)) -> true
    | ((Var' (_, Param _), _), _) -> false
    | (_, (Var' (_, Param _), _)) -> false
    | ((Var' (_, Bound (rm,_)), re),
      (Var' (_, Bound (wm,_)), we)) ->
        (List.nth re rm) == (List.nth we wm)
    | _ -> raise
      (X_this_should_not_happen "free vars shouldnt be here")
    in
    let same_cross_occ = List.map same_occ cross in
    List.exists (fun occ -> occ == false) same_cross_occ
  ...
  | ScmApplic' (proc, args, app_kind) ->
     ...
     let args =
       Reader.scheme_sexpr_list_of_sexpr_list
         (List.map sexpr_of_expr' args) in
     ScmPair (proc, args)
  | ScmBox' (var') -> sexpr_of_var' var'
  | ScmBoxGet' (var') -> sexpr_of_var' var'
  | ScmBoxSet' (var', expr') -> ScmPair (ScmSymbol "set!", ScmPair(sexpr_of_var' var', ScmPair(sexpr_of_expr' expr', ScmNil)))
  | _ -> raise (X_this_should_not_happen "Unknown expr'");;
